...
function generateMethod(requestName, operation, options) {
    const result = [];
...
function generateAPI(docs) {
    ...
    result.push(`get({ path, ...options}: any): Promise<any> {`, `options.url = !path ? options.url : Object.keys(path).reduce((memo, key) => memo.replace(new RegExp('{' + key + '}'), path[key]), options.url);`, `return this.call(options);`, `}`);
    return [
        typesResult.join('\n'),
